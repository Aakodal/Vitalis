...
class Client extends Discord.Client {
    ...
    async loadCommand(commandName: string) {
        try {
            const { default: importCommand } = await import(`../commands/${commandName}`);
            const command = new importCommand();
            if(!command.name) return console.log(`Command in ${commandName} doesn't have any name. Skipping...`)
            if(this.commands.has(command.name)) return console.log(`Command ${command.name} in ${commandName} already exists. Skipping...`);
            this.commands.set(command.name, command);
            console.log(`Command ${commandName} loaded.`);
            command.aliases.forEach((alias: string) => {
                if(this.aliases.has(alias)) return console.log(`Alias ${alias} already exist for command ${this.aliases.get(alias)}.`);
