...
export async function getLeavingMessageChannelEmbed(message: Message): Promise<MessageEmbed> {
	const channelId = await getValueFromDB<string>("servers", "leaving_message_channel", {
		server_id: message.guild?.id,
	});
	const channel = fetchGuildChannel(message.guild as Guild, channelId) || "<no channel defined>";
	return new MessageEmbed()
		.setAuthor(
			"Configuration Editor - Leaving message channel",
