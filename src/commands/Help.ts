...
export default class Help extends Command {
    ...
    async run(message: Discord.Message, args: any[], client: Client) {
        ...
        if(!args[0] || !isNaN(args[0])) {   // Global help
            ...
            client.commands.forEach((command: any) => {
                if(command.permission === "BOT_OWNER") {
                    if(message.author.id === config.botOwner) commands.push({category: command.category, name: command.name});
                    return;
                }
                if(command.permission) {
                    if(message.member.hasPermission(command.permission)) commands.push({category: command.category, name: command.name});
                    return;
                } 
                commands.push({category: command.category, name: command.name});
            });
            ...
            let currentPage = args[0] && !isNaN(args[0])
